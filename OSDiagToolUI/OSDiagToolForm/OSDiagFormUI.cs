...
namespace OSDiagToolUI.OSDiagToolForm {
    public partial class OsDiagForm : Form {
        ...
        public OsDiagForm(ConfModel.strConfModel configurations, DatabaseType dbms, SQLConnStringModel SQLConnectionString = null, OracleConnStringModel OracleConnectionString = null) {
            ...
            if (ConfigFileReader.dbLoggingAvailable.Equals(true))
            {
        ...
        private void bt_TestSaConnection_Click(object sender, EventArgs e, DatabaseType dbms, SQLConnStringModel SQLConnectionString = null, OracleConnStringModel OracleConnectionString = null) {

            string testConnectionResult = null;

            if (dbms.Equals(DatabaseType.SqlServer)) {
                SQLConnectionString.userId = this.tb_iptSaUsername.Text;
                SQLConnectionString.pwd = this.tb_iptSaPwd.Text;

                try {
                    var connector = new SLQDBConnector();
        ...
        private void bt_runOsDiagTool_Click(object sender, EventArgs e, ConfModel.strConfModel configurations) {
            ...
            configurationsHelper.ConfigFileConfigurations = new ConfModel.strConfModel();
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e) {
            ...
            bool useMultiThread = OSDiagTool.Core.Program.useMultiThread;

            // Multi thread refactor
            int numberOfTasks = 1;
            CountdownEvent countdown = new CountdownEvent(numberOfTasks);
            ThreadPool.SetMaxThreads(OSDiagTool.Core.Program.serverProcessorCount, OSDiagTool.Core.Program.serverProcessorCount); // cannot set the maximum number of worker threads or I/O completion threads to a number smaller than the number of processors on the computer
            FileLogger.TraceLog("Server Processor count: " + OSDiagTool.Core.Program.serverProcessorCount);
            ...
            if (!backgroundWorker1.CancellationPending) {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._plPlatformAndServerFiles, out bool getPSandServerConfFiles) && getPSandServerConfFiles == true) {
                    ...
                    if (useMultiThread) {
                        numberOfTasks++;
                        countdown.AddCount();
                        ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.GetPlatformAndServerFiles(countdown));

                    } else {
                        OSDiagTool.Core.Program.GetPlatformAndServerFiles();
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending) {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._plPlatformIntegrity, out bool performPlatIntegrity) && performPlatIntegrity == true) {
                    ...
                    OSDiagTool.Core.Platform.PlatformConnectionStringDefiner ConnectionStringDefiner_pdic = new PlatformConnectionStringDefiner();
                    PlatformConnectionStringDefiner ConnStringHelper_pdic = ConnectionStringDefiner_pdic.GetConnectionString(Program.dbEngine, false, false, ConnectionStringDefiner_pdic);

                    if (useMultiThread)
                    {
                        numberOfTasks++;
                        countdown.AddCount();
                        ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.PlatformIntegritycheck(configurationsHelper.ConfigFileConfigurations, ConnectionStringDefiner_pdic.SQLConnString, ConnectionStringDefiner_pdic.OracleConnString, ConnStringHelper_pdic.AdminSchema, countdown));

                    }
                    else
                    {
                        OSDiagTool.Core.Program.PlatformIntegritycheck(configurationsHelper.ConfigFileConfigurations, ConnectionStringDefiner_pdic.SQLConnString, ConnectionStringDefiner_pdic.OracleConnString, ConnStringHelper_pdic.AdminSchema, countdown);
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending) {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._slEvt, out bool getEvt) && getEvt == true) {
                    ...
                    if (useMultiThread) {
                        countdown.AddCount();
                        ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.ExportEventViewerAndServerLogs(countdown: countdown));
                        numberOfTasks++;
                    }
                    else {
                        OSDiagTool.Core.Program.ExportEventViewerAndServerLogs(); 
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending) {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._slIisLogs, out bool getIisLogs) && getIisLogs == true) {
                    ...
                    {
                        countdown.AddCount();
                        ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.CopyIISAccessLogs(configurationsHelper.FormConfigurations.iisLogsNrDays, countdown));
                        numberOfTasks++;
                    }
                    else
                    {
                        OSDiagTool.Core.Program.CopyIISAccessLogs(configurationsHelper.FormConfigurations.iisLogsNrDays);
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending) {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._tdIis, out bool getIisThreadDumps) && getIisThreadDumps == true) {
                    ...
                    {
                        countdown.AddCount();
                        ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.CollectThreadDumpsProgram(getIisThreadDumps, false, countdown));
                        numberOfTasks++;
                    } else
                    {
                        OSDiagTool.Core.Program.CollectThreadDumpsProgram(getIisThreadDumps, false, listIISRequests: listIISRequests);
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending) {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._tdOsServices, out bool _getOsThreadDumps) && _getOsThreadDumps == true) {
                    ...
                    {
                        countdown.AddCount();
                        ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.CollectThreadDumpsProgram(false, _getOsThreadDumps, countdown));
                        numberOfTasks++;
                    } else
                    {
                        OSDiagTool.Core.Program.CollectThreadDumpsProgram(false, _getOsThreadDumps);
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending) {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._mdIis, out bool getIisMemDumps) && getIisMemDumps == true) {
                    ...
                    {
                        countdown.AddCount();
                        ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.CollectMemoryDumpsProgram(getIisMemDumps, false, countdown));
                        numberOfTasks++;
                    } else
                    {
                        OSDiagTool.Core.Program.CollectMemoryDumpsProgram(getIisMemDumps, false);
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending) {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._mdOsServices, out bool getOsMemDumps) && getOsMemDumps == true) {
                    ...
                    {
                        countdown.AddCount();
                        ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.CollectMemoryDumpsProgram(false, getOsMemDumps, countdown));
                        numberOfTasks++;
                    } else
                    {
                        OSDiagTool.Core.Program.CollectMemoryDumpsProgram(false, getOsMemDumps);
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending)
            {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._osDiagnostic, out bool getOsDiagnostic) && getOsDiagnostic == true)
                {
                    ...
                    // Connecting to the database, by using the credentials located in the server.hsconf
                    PlatformConnectionStringDefiner ConnectionStringDefiner_pd = new PlatformConnectionStringDefiner();
                    PlatformConnectionStringDefiner ConnStringHelper_pd = ConnectionStringDefiner_pd.GetConnectionString(Program.dbEngine, false, false, ConnectionStringDefiner_pd);
                    ...
                    if (useMultiThread)
                    {
                        ...
                        {
                            ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.PlatformDiagnosticProgram(configurationsHelper.ConfigFileConfigurations, ConnStringHelper_pd.SQLConnString, null, countdown));
                        }
                        else if (Program.dbEngine.Equals(DatabaseType.Oracle))
                        {
                            ThreadPool.QueueUserWorkItem(work => OSDiagTool.Core.Program.PlatformDiagnosticProgram(configurationsHelper.ConfigFileConfigurations, null, ConnStringHelper_pd.OracleConnString, countdown));
                        }
                    } else
                    {
                        if (Program.dbEngine.Equals(DatabaseType.SqlServer))
                        {
                            OSDiagTool.Core.Program.PlatformDiagnosticProgram(configurationsHelper.ConfigFileConfigurations, ConnStringHelper_pd.SQLConnString, null);
                        }
                        else if (Program.dbEngine.Equals(DatabaseType.Oracle))
                        {
                            OSDiagTool.Core.Program.PlatformDiagnosticProgram(configurationsHelper.ConfigFileConfigurations, null, ConnStringHelper_pd.OracleConnString);
                        }
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending)
            {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._diMetamodel, out bool _getPlatformMetamodel) && _getPlatformMetamodel == true)
                {
                    ...
                    PlatformConnectionStringDefiner ConnectionStringDefiner_pm = new PlatformConnectionStringDefiner();
                    PlatformConnectionStringDefiner ConnStringHelper_pm = ConnectionStringDefiner_pm.GetConnectionString(Program.dbEngine, false, false, ConnectionStringDefiner_pm);

                    if (Program.dbEngine.Equals(DatabaseType.SqlServer))
                    {
                        OSDiagTool.Core.Program.ExportPlatformMetamodel(OSDiagTool.Core.Program.dbEngine, configurationsHelper.ConfigFileConfigurations, ConnStringHelper_pm.SQLConnString, null);
                    }
                    else if (Program.dbEngine.Equals(DatabaseType.Oracle))
                    {
                        OSDiagTool.Core.Program.ExportPlatformMetamodel(OSDiagTool.Core.Program.dbEngine, configurationsHelper.ConfigFileConfigurations, null, ConnStringHelper_pm.OracleConnString);
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending)
            {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._plPlatformLogs, out bool _getPlatformLogs) && _getPlatformLogs == true)
                {
                    ...
                    PlatformConnectionStringDefiner ConnectionStringDefiner_pl = new PlatformConnectionStringDefiner();
                    PlatformConnectionStringDefiner ConnStringHelper_pl = ConnectionStringDefiner_pl.GetConnectionString(Program.dbEngine, Program.separateLogCatalog, false, ConnectionStringDefiner_pl);

                    if (Program.dbEngine.Equals(DatabaseType.SqlServer))
                    {
                        OSDiagTool.Core.Program.ExportServiceCenterLogs(Program.dbEngine, configurationsHelper.ConfigFileConfigurations, ConnStringHelper_pl.SQLConnString, null);
                    }
                    else if (Program.dbEngine.Equals(DatabaseType.Oracle))
                    {
                        OSDiagTool.Core.Program.ExportServiceCenterLogs(Program.dbEngine, configurationsHelper.ConfigFileConfigurations, null, ConnStringHelper_pl.OracleConnString, ConnStringHelper_pl.AdminSchema);
                    }
                }
            }
            ...
            if (!backgroundWorker1.CancellationPending)
            {
                if (configurationsHelper.FormConfigurations.cbConfs.TryGetValue(OSDiagToolForm.OsDiagForm._diDbTroubleshoot, out bool _doDbTroubleshoot) && _doDbTroubleshoot == true)
                {
                    ...
                    PlatformConnectionStringDefiner ConnectionStringDefiner_dt = new PlatformConnectionStringDefiner();
                    PlatformConnectionStringDefiner ConnStringHelper_dt = ConnectionStringDefiner_dt.GetConnectionString(Program.dbEngine, false, true, ConnectionStringDefiner_dt, configurationsHelper.FormConfigurations.saUser, configurationsHelper.FormConfigurations.saPwd);

                    if (Program.dbEngine.Equals(DatabaseType.SqlServer))
                    {
                        OSDiagTool.Core.Program.DatabaseTroubleshootProgram(configurationsHelper.ConfigFileConfigurations, ConnStringHelper_dt.SQLConnString);
                    }
                    else if (Program.dbEngine.Equals(DatabaseType.Oracle))
                    {
                        OSDiagTool.Core.Program.DatabaseTroubleshootProgram(configurationsHelper.ConfigFileConfigurations, null, ConnStringHelper_dt.OracleConnString);
                    }
                }
            }
            ...
            OSDiagTool.Core.Program.GenerateZipFile();
