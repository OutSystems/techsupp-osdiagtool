    ...
    public class Program
    {
        ...
        static void Main(string[] args) {
            ...
            if(osPlatformVersion == null) {
                Console.WriteLine("OutSystems Platform installation not found");
                Console.ReadLine();
            }
            else {
        ...
        public static void ExportPlatformMetamodel(Database.DatabaseType dbEngine, OSDiagToolConf.ConfModel.strConfModel configurations,
        ...
        public static void ExportServiceCenterLogs(Database.DatabaseType dbEngine, OSDiagToolConf.ConfModel.strConfModel configurations,
            ...
            try
            {
                FileLogger.TraceLog(string.Format("Exporting Platform logs ({0} records)...", configurations.osLogTopRecords));
                ...
                if (dbEngine.Equals(Database.DatabaseType.SqlServer))
                {
                    Platform.LogExporter.PlatformLogExporter(dbEngine, platformLogs, configurations, _osPlatformLogs, configurations.queryTimeout, sqlConnString, null);
                }
                else if (dbEngine.Equals(Database.DatabaseType.Oracle))
                {
                    Platform.LogExporter.PlatformLogExporter(dbEngine, platformLogs, configurations, _osPlatformLogs, configurations.queryTimeout, null, oracleConnString, adminSchema);
                }
            }
            catch (Exception e)
            {
