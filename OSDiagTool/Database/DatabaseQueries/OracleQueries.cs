...
namespace OSDiagTool.Database.DatabaseQueries {
    class OracleQueries {
        ...
        public string lockedObjects { get; set; } = @"SELECT * FROM V$LOCKED_OBJECT";
        ...
        public string sidInfo { get; set; } = @"SELECT * FROM V$SESSON s JOIN V$PROCESS p ON p.ADDR = s.PADDR WHERE s.SID = {0}"; // {0}: SID

        public string resourceLimit { get; set; } = @"SELECT * FROM V$RESOURCE_LIMIT";

        public string sessionByIOType { get; set; } = @"select p.spid, s.sid, s.process cli_process, s.status, t.{0}, s.last_call_et/3600 last_call_et_Hrs, s.action, s.program, lpad(t.sql_text,30)
        ...
        public string tk_queriesRunning { get; set; } = @"
            with q as (
            select sql_id from(
